...
class InterestOperation
{
  public:
    ...
    InterestOperation(Client *client, unsigned long timeout,
                      ...
                      doid_t parent, std::unordered_set<zone_t> zones, bool send_interest_done, channel_t caller);
    ~InterestOperation();
...
class Client : public MDParticipantInterface
{
  ...
  protected:
    ...
    void add_interest(Interest &i, uint32_t context, bool send_interest_done, channel_t caller = 0);
    ...
    void remove_interest(Interest &i, uint32_t context, bool send_interest_done, channel_t caller = 0);
