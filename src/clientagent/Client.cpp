...
void Client::add_interest(Interest &i, uint32_t context, bool send_interest_done, channel_t caller)
{
    ...
    InterestOperation *iop = new InterestOperation(this, m_client_agent->m_interest_timeout,
            i.id, context, request_context, i.parent, new_zones, send_interest_done, caller);
...
void Client::remove_interest(Interest &i, uint32_t context, bool send_interest_done, channel_t caller)
{
    unordered_set<zone_t> killed_zones;
...
void Client::handle_datagram(DatagramHandle in_dg, DatagramIterator &dgi)
{
    ...
    case CLIENTAGENT_ADD_INTEREST: {
        ...
        bool send_interest_done = dgi.read_bool();
        handle_add_interest(i, context);
        add_interest(i, context, send_interest_done, sender);
    }
    break;
    ...
    case CLIENTAGENT_REMOVE_INTEREST: {
        ...
        remove_interest(i, context, send_interest_done, sender);
    }
    break;
...
InterestOperation::InterestOperation(
    ...
    doid_t parent, unordered_set<zone_t> zones, bool send_interest_done, channel_t caller) :
    m_client(client),
    ...
    m_send_interest_done(send_interest_done)
{
